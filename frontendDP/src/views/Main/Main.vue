...
export default {
    ...
    methods: {
      ...
      getDimensionScale(keyMap, width, dimNum, parentKeyList) {
        let keyLen = []
        ...
        let parentNodeWidths = [];
        let nodeWidths = [];
        for(let i = 0 ;i< dimNum;i++) {
          let parentNodeWidth = width / keyLen[i];
          let nodeWidth = width / dimensionNodeNum[i];
          parentNodeWidths.push(parentNodeWidth);
          nodeWidths.push(nodeWidth);
          parentNodeScaleList.push(
              d3.scaleBand()
                  .domain(keyMap[i].data)
                  .range([parentNodeWidth / 2, width + parentNodeWidth / 2])
          )

          childNodeScaleList.push(
              d3.scaleBand()
                  .domain(Array.from(parentKeyList[i]))
                  .range([-parentNodeWidth/2 + nodeWidth / 2, parentNodeWidth/2 - nodeWidth / 2])
          )
        }

        return [parentNodeScaleList, childNodeScaleList, parentNodeWidths, nodeWidths]
